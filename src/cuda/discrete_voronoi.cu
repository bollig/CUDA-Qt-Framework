// Originally from Thrust 1.5 examples (tested with Thrust v1.4 (CUDA 4.0))
...
// print an M-by-N array
    template <typename T>
void Voronoi::print(int m, int n, const thrust::device_vector<T>& d_data)
{
...
float Voronoi::timer_stop_and_display()
{
    float elapsed_time;
...
void Voronoi::generate_random_sites(thrust::host_vector<int> &t, int Nb, int m, int n)
{
...
void Voronoi::vector_to_pgm(thrust::host_vector<int> &t, int m, int n,char *out)
{
    FILE *f;
...
void Voronoi::jfa(thrust::device_vector<int>& in,thrust::device_vector<int>& out, unsigned int k, int m, int n)
{
    thrust::transform(
